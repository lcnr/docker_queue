...
async fn queue_container_runs_if_no_running_containers() {
    ...
    let mut app = spawn_app().await;
    let command =
        "docker run -d --rm alpine sh -c \"sleep 30 && echo queue_container_runs_if_no_running_containers\"";
...
async fn queue_container_queues_if_already_running() {
    ...
    let mut app = spawn_app().await;
    let command1 =
        "docker run -d --rm alpine sh -c \"sleep 30 && echo queue_container_queues_if_already_running1\"";
    let command2 =
        "docker run -d --rm alpine sh -c \"sleep 30 && echo queue_container_queues_if_already_running2\"";
